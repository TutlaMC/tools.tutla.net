...
export default function Home() {
  ...
  const [name, setName] = useState("Xylo_jaX");
  const [color, setColor] = useState("white");
  const [time, setTime] = useState("9/3/25, 9:24 PM");
  ...
  const [text, setText] = useState("who studied for tm's test 8b");
  ...
  return (
    ...
    <div className="min-h-screen bg-[#2b2d31] text-white p-6">
      ...
      <div className="space-y-3 mb-6">
        ...
        <input className="w-full p-2 rounded bg-[#1e1f22]" value={color} onChange={e => setColor(e.target.value)} placeholder="Name" />
      ...
      <img src={apiUrl} alt="preview" className="rounded bg-[#313338] p-4" />
    </div>
  );
}
